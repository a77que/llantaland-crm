...
// Corrección de datos: precioProveedor y precioReferencialVenta quedaron
// cruzados en todo el catálogo (import antiguo). El swap es una sola sentencia
// SQL — Postgres evalúa el SET con los valores originales de la fila, así que
// es atómico y, si se ejecuta dos veces, revierte el cambio (auto-invertible).
// Después recalcula precioOferta y precioRegular porque dependían del valor
// (equivocado) de precioProveedor.
const intercambiarProveedorReferencial = async (req, res, next) => {
  try {
    const swap = await prisma.$executeRaw`
      UPDATE "productos"
      SET "precioProveedor" = "precioReferencialVenta",
          "precioReferencialVenta" = "precioProveedor"
    `;

    const costos = await prisma.costoVenta.findMany({ where: { activo: true } });
    const productos = await prisma.producto.findMany({
      where: { precioProveedor: { not: null } },
      select: { id: true, precioProveedor: true },
    });
    let recalculados = 0;
    const BATCH = 100;
    for (let i = 0; i < productos.length; i += BATCH) {
      ...
      await prisma.$transaction(
        lote.map(p => {
          ...
          const precioOferta = Math.round((base + total) * 100) / 100;
          const precioRegular = Math.round(precioOferta * 1.05 * 100) / 100;
          return prisma.producto.update({ where: { id: p.id }, data: { precioOferta, precioRegular } });
        })
      );
      recalculados += lote.length;
    }
    invalidarCachePrecios();
    res.json({ ok: true, filasIntercambiadas: swap, recalculados });
  } catch (err) {
    next(err);
  }
};

module.exports = { listar, obtener, crear, actualizar, eliminar, eliminarMasivo, eliminarPorSku, compatibles, subirImagen, marcas, tipos, medidas, enriquecerConIA, hermanasImagen, aplicarImagen, gruposImagen, subirImagenMultiple, exportFaltantesImagen, incompletos, enriquecerMasivo, sincronizarPrecioRegular, recalcularPrecioOferta, intercambiarProveedorReferencial };
